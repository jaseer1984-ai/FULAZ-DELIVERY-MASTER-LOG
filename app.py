...
def _excel_file_from_upload(uploaded_file):
    """Return (pd.ExcelFile, extension) with proper engine (.xlsx -> openpyxl, .xls -> xlrd)."""
    file_bytes = uploaded_file.getvalue() if hasattr(uploaded_file, "getvalue") else uploaded_file.read()
    name = getattr(uploaded_file, "name", "") or ""
    is_xls = name.lower().endswith(".xls")
    try:
        if is_xls:
            # needs xlrd==2.0.1 in requirements.txt
            return pd.ExcelFile(io.BytesIO(file_bytes), engine="xlrd"), ".xls"
        else:
            # .xlsx uses openpyxl
            return pd.ExcelFile(io.BytesIO(file_bytes), engine="openpyxl"), ".xlsx"
    except ImportError as e:
        if is_xls:
            st.error("This is a .xls file. Ensure `xlrd==2.0.1` is in requirements.txt or resave as .xlsx.")
        else:
            st.error("This is a .xlsx file. Ensure `openpyxl` is installed.")
        raise
...
def load_excel(file, sheet_name: Optional[str] = None) -> pd.DataFrame:
    ...
    # Path/bytes (rare on Streamlit Cloud)
    if isinstance(file, (str, bytes)):
        path = file if isinstance(file, str) else ""
        is_xls = path.lower().endswith(".xls")
        engine = "xlrd" if is_xls else "openpyxl"
        xobj = pd.ExcelFile(file, engine=engine)
        return read_sheet(xobj, sheet_name)

    # UploadedFile
    xobj, _ext = _excel_file_from_upload(file)
    return read_sheet(xobj, sheet_name)

def extract_header_dates(file, sheet_name: Optional[str] = None) -> List[tuple]:
    """Extract date-like cells from first row only (works for both formats)."""
    try:
        if isinstance(file, (str, bytes)):
            path = file if isinstance(file, str) else ""
            is_xls = path.lower().endswith(".xls")
            xobj = pd.ExcelFile(file, engine=("xlrd" if is_xls else "openpyxl"))
        else:
            xobj, _ext = _excel_file_from_upload(file)
...
# Peek sheet names robustly
try:
    peek_xls, _ext = _excel_file_from_upload(uploaded)
    sheet_options = peek_xls.sheet_names
except Exception:
...
# Normalize progress to 0–1 if given as 0–100
...
# Safety init for trends tab usage
    ...
    if header_dates:
        ...
        if "ALL DATES" not in selected_header_dates:
            ...
            if selected_date_values:
                ...
                # (Optional) If your file encodes quantities under those header columns,
                # you can add logic here to select those columns for analysis.
...
with st.expander("💡 DASHBOARD USAGE GUIDE", expanded=False):
    ...
    - **ALL CAPS**: PROFESSIONAL REPORT PRESENTATION
    - **EXPORTS**: FILTERED CSV, SUMMARY, TRUCK & ZONE ANALYSIS
    """)
